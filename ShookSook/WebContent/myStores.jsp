<%@ page contentType="text/html; charset=utf-8" %>

<!DOCTYPE html>

<jsp:useBean id="db" class = "jdbc.ForDataBase"/>

<html>

<%@include file="include/head.jsp" %>

<body class="body">
	
<img src="images/bg1.jpg" alt="" class = "bg">

<div id = "page-wrap">

	<header class="mainHeader">
		<%@include file="include/header.jsp" %>		
		
		<nav><ul>
			<li><a href="index.jsp">Home</a></li>
			<li class="active"><a href="stores.jsp">Stores</a></li>
			<li><a href="holy.jsp">Holy Places</a></li>
			<li><a href="map.jsp">Map</a></li>
			<li><a href="media.jsp">Media</a></li>
			<li><a href="game.jsp">Game</a></li>
		</ul></nav>
	</header>
		
	<div class="content">	
	
		<article class="article">
			<header  id = "searchbox">
				<form action="resultStores.jsp" method="GET">  		
  					<input id = "search-field" type="search" name = "input" placeholder="Search.." />
    				<input id = "search-submit" class = "submit-decorate" type="submit" value="Search" />
				</form>
			</header>
			
			<header class = "stores-show">
				<h2><a href="stores.jsp" title="All Stores">All Stores</a></h2>
			</header>

			<header class = "stores-show">
				<h2>My Stores</h2>
			</header>
			
			<header class = "stores-show">
				<h2><a href = addStore.jsp> Add your own Store</a></h2>
			</header>
		</article>
				
<%
if (session.getAttribute("login_success") != "true")
{
%>
		<article class="article">	
			<header>
				<h2 class = "colored">Login Required</h2>
			</header>
			
			<div class = "content-textblock">
				<p>You need to be logged in in order to view this page.</p>
			</div>
		</article>
		
<%
}
else
{

String fullName = ((String [])session.getAttribute("user"))[4] + " " + ((String [])session.getAttribute("user"))[5];
String sqlStr = "SELECT * FROM stores WHERE creator = '" + fullName + "';";
String [][] stores = db.select(sqlStr);

		if (stores.length == 0)
		{
			%>
		<article class="article">
			<div class = "content-textblock">
				<p>You didn't submit any stores yet!</p>
			</div>
		</article>
 			<%
		}
		else
		{
			for (int i = 0; i <stores.length; i++)
			{
				%>
		<article class = "article">
			<div class = "content-imgbox">
				<img class = "content-img" src = "<%= stores[i][6] %>">
			</div>
			
			<header>
				<form class = "store-change" id = "full-store" action="store_page.jsp" method="GET">
					<input type="hidden" name="storeId" value= <%= stores[i][0] %>>
	   				<input type="submit" class = "submit-none title colored" value= "<%=stores[i][2] %>" />
				</form>
			</header>
			
			<footer class="post-info">
				<p><%= stores[i][3] %></p>
				<p><%= stores[i][4] %></p>
				<p>Added by <%= stores[i][1] %></p>
			</footer>
			
			<div class = "content-textblock">
				<p><%= stores[i][5] %></p>
			</div>
			
			<div class = "store-buttons">
				<form class = "store-change" id = "edit-store" action="storeEdit.jsp" method="GET">
					<input type="hidden" name="storeId" value= <%= stores[i][0] %>>
    				<input type="submit" class = "submit-decorate" value="Edit Store" />
				</form>
  			
				<form class = "store-change" id = "delete-store" action="jsp/store_delete.jsp" method="POST">
					<input type="hidden" name="storeId" value= <%= stores[i][0] %>>
    				<input name = <%= stores[i][2] %> class = "submit-decorate" type="submit" value="Delete Store" />
				</form>
			</div>
		</article>
				<%
			}
		}
}
		%>
		
	</div>
	
	<%@include file="include/footer.jsp" %>		
	
</div>

</body>
</html>